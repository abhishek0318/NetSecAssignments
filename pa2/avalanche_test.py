...
class DES():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False, debug=False):
        ...
        if debug:
            debug_result.append(''.join(map(str, block)))
        ...
        for i in range(16):  # Do the 16 rounds
            ...
            d = tmp
            if debug:
                debug_result.append(''.join(map(str, g+d)))
...
def avalanche_test(key1, plain1, key2, plain2):

    ciphertext_allround1 = cipher.run(key1, plain1, ENCRYPT, debug=True)
    ciphertext_allround2 = cipher.run(key2, plain2, ENCRYPT, debug=True)
    ...
    print("Plaintext", ciphertext_allround1[0], ciphertext_allround2[0], diff_count(ciphertext_allround1[0], ciphertext_allround2[0]), sep='\t')

    for i in range(1, 17):
        print("Round {}:".format(i), ciphertext_allround1[i], ciphertext_allround2[i], diff_count(ciphertext_allround1[i], ciphertext_allround2[i]), sep='\t')


if __name__ == '__main__':
    cipher = DES()

    avalanche_test(key1 = '0110110101101111011011100110000101110010011000110110100001111001',\
                    plain1 = '0100100001100101011011000110110001101111001000000111011101101111',\
                    key2 = '0110110101101111011011100110000101110010011000110110100001111001',\
                    plain2 = '1100100001100101011011000110110001101111001000000111011101101111')

    print()

    avalanche_test(key1 = '0110110101101111011011100110000101110010011000110110100001111001',\
                    plain1 = '0100100001100101011011000110110001101111001000000111011101101111',\
                    key2 = '1110110101101111011011100110000101110010011000110110100001111001',\
                    plain2 = '0100100001100101011011000110110001101111001000000111011101101111')
