...
class DES():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        ...
        Args:
            key: string with binary digits of length 64
            text: string with binary digits of length 64 (if padding is False)
            action: ENCRYPT or DECRYPT
            padding: True or False

        Returns:
            encrypted_text: string of binary digits with same length as text
        """
        ...
        if padding and action == DECRYPT:
            ...
            return ''.join([str(bit) for bit in self.removePadding(result)])
        else:
